...
impl NonEmptyLinkedList {
    ...
    /// Adds an element to the end list with O(1) computation time.
    /// Instead of creating a new list, this mutates the current list
    fn push_mut(&mut self, item: i32) {
        self.next = Some(Box::new(NonEmptyLinkedList {
            num: self.num, 
            next: self.next.take()
        }));
        self.num = item;
    ...
    fn to_string(&self) -> String {
        let mut nums = vec![self.num];

        let mut prev_node = self;
        while let Some(node) = &prev_node.next {
            nums.push(node.num);
            prev_node = node;
        }
    ...
    /// to_string, but does no redundant string allocations
    fn to_string_optimized(&self) -> String {
        use std::fmt::Write;
        ...
        write!(&mut output, "{}]", nums.first().unwrap()).unwrap();
        output
    }
...
#[test]
fn test_problem_2_push_mut () {
    // test push()
    let mut l = NonEmptyLinkedList::new(1);
    l.push_mut(2);
    l.push_mut(3);
...
let l = NonEmptyLinkedList::new(1);
let l = l.push(2).push(3);
assert_eq!(l.to_string(), "[1, 2, 3]");
}
#[test]
fn test_problem_2_to_string_optimized () {
    let l = NonEmptyLinkedList::new(1).push(2).push(3);
    assert_eq!(l.to_string_optimized(), "[1, 2, 3]");
}
#[test]
...
fn test_problem_3_b() {
    ...
    assert_eq!(primes_between(10, 20), [11, 13, 17, 19]);
}
...
fn primes_between_parallel(start: u64, end: u64) -> Vec<u64> {
    use rayon::prelude::*;
    let now = std::time::Instant::now();
    let primes = (start..end).into_par_iter().filter(|n| is_prime(*n)).collect();
    println!("Elapsed {} ms", now.elapsed().as_millis());
    return primes
}
